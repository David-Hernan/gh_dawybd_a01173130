...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-10-2021 a las 20:07:46
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `registrarEntrega` (IN `uClave` INT, IN `RFC` VARCHAR(15) CHARSET utf8, IN `Numero` INT, IN `Cantidad` INT)  BEGIN
INSERT INTO Entregan VALUES(uClave,RFC,Numero,CURRENT_DATE(),Cantidad);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `registrarMaterial` (IN `Clave` INT, IN `Descripcion` VARCHAR(40) CHARSET utf8, IN `Precio` FLOAT, IN `Impuesto` FLOAT, IN `PorcentajeImpuesto` DECIMAL)  INSERT INTO materiales
VALUES (Clave,Descripcion,Precio,Impuesto,PorcentajeImpuesto)$$
...
(1000, 'AAAA800101', 5000, '2021-10-20', 300),
...
(1500, 'Blocks negros', 12, 32, '2.00'),
(1501, 'Cristal templado', 23, 32, '2.00'),
(1502, 'Azulejos', 34, 32, '2.10'),
(1503, 'Madera', 45, 32, '2.00'),
(1504, 'Cemento', 12, 32, '2.00'),
(2000, 'Jabón', 125, 12.5, '4.00'),
(6010, 'Arcilla decorativa', 120, 20.5, '2.00');

-- --------------------------------------------------------

--
-- Estructura Stand-in para la vista `nomanches`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `nomanches` (
`clave` int(11)
...
--
-- Estructura Stand-in para la vista `proveedores_razonsocial`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `proveedores_razonsocial` (
`RFC` varchar(15)
);
...
CREATE TABLE `t1` (
  `d` varchar(16) COLLATE utf8mb4_spanish_ci DEFAULT NULL
...
--
-- Estructura para la vista `nomanches`
--
DROP TABLE IF EXISTS `nomanches`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `nomanches`  AS SELECT `materiales`.`clave` AS `clave`, `materiales`.`descripcion` AS `descripcion`, `materiales`.`precio` AS `precio`, `materiales`.`impuesto` AS `impuesto`, `materiales`.`PorcentajeImpuesto` AS `PorcentajeImpuesto` FROM `materiales` WHERE `materiales`.`clave` = 1000 ;

-- --------------------------------------------------------

--
-- Estructura para la vista `proveedores_razonsocial`
--
DROP TABLE IF EXISTS `proveedores_razonsocial`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `proveedores_razonsocial`  AS SELECT `entregan`.`rfc` AS `RFC` FROM (`proveedores` join `entregan`) WHERE `proveedores`.`razonsocial` like 'La%' AND `entregan`.`rfc` = `proveedores`.`rfc` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista1`  AS SELECT `entregan`.`rfc` AS `RFC` FROM (`proveedores` join `entregan`) WHERE `proveedores`.`razonsocial` like 'La%' AND `entregan`.`rfc` = `proveedores`.`rfc` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista2`  AS SELECT sum(`e`.`cantidad` * ((1 + `m`.`PorcentajeImpuesto` / 100) * `m`.`precio`)) AS `Importe de entregas` FROM (`materiales` `m` join `entregan` `e`) WHERE `m`.`clave` = `e`.`clave` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista3`  AS SELECT `entregan`.`clave` AS `Clave`, `entregan`.`rfc` AS `RFC`, `entregan`.`numero` AS `Numero`, `entregan`.`fecha` AS `Fecha`, `entregan`.`cantidad` AS `Cantidad` FROM `entregan` WHERE `entregan`.`fecha` between '2001-01-01' and '2001-12-31' ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista5`  AS SELECT `entregan`.`rfc` AS `RFC` FROM (`proveedores` join `entregan`) WHERE `proveedores`.`razonsocial` not like 'La%' AND `entregan`.`rfc` = `proveedores`.`rfc` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista6`  AS SELECT `materiales`.`clave` AS `clave` FROM `materiales` WHERE `materiales`.`descripcion` like 'Si%' ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista7`  AS SELECT `entregan`.`clave` AS `clave` FROM `entregan` WHERE extract(year from `entregan`.`fecha`) = 2001 GROUP BY `entregan`.`clave` ORDER BY sum(`entregan`.`cantidad`) DESC ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista8`  AS SELECT `p`.`denominacion` AS `denominacion`, `e`.`rfc` AS `rfc`, `v`.`razonsocial` AS `razonsocial` FROM ((`entregan` `e` join `proyectos` `p`) join `proveedores` `v`) WHERE `e`.`numero` = `p`.`numero` AND `v`.`rfc` = `e`.`rfc` AND `p`.`denominacion` = 'Televisa en acción' ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista9`  AS SELECT `p`.`denominacion` AS `denominacion`, `e`.`rfc` AS `rfc`, `v`.`razonsocial` AS `razonsocial` FROM ((`entregan` `e` join `proyectos` `p`) join `proveedores` `v`) WHERE `e`.`numero` = `p`.`numero` AND `v`.`rfc` = `e`.`rfc` AND `p`.`denominacion` = 'Educando en Coahuila' ;
